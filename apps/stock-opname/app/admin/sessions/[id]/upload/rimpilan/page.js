...
// Upload Rimpilan Master — sama pola dengan Upload Data SAP
// (app/admin/sessions/[id]/upload/page.js) tapi menulis ke
// rimpilan_sap_data, dan wajib punya Nomor Rak + Level (dipakai untuk
// accordion di halaman input rimpilan).
...
export default function UploadRimpilanMasterPage() {
  ...
  function handleFile(e) {
    ...
    reader.onload = () => {
      try {
        ...
        // Material Description opsional. Level opsional (default 1 kalau
        // kosong — petugas bisa koreksi saat input fisik). Sisanya wajib,
        // termasuk Nomor Rak karena itu dasar dari accordion per-rak.
        const requiredFields = ['material', 'batch', 'base_uom', 'plant', 'storage_location', 'material_group', 'qty', 'nomor_rak'];
        ...
        for (const r of rawRows) {
          ...
          const level = Number.isFinite(levelRaw) && levelRaw >= 1 && levelRaw <= 7 ? levelRaw : 1;
  ...
  async function handleUpload() {
    ...
    if (uploadMode === 'ganti' && totalCount > 0) {
      if (!confirm(`Hapus ${totalCount} baris Master Rimpilan lama dan ganti dengan ${parsedRows.length} baris baru?`)) return;
    }
    setUploading(true);
    setProgress(0);
    setError(null);
    ...
    try {
      ...
      if (failedChunks.length > 0 || actualCount !== expectedCount) {
        ...
        setTimeout(() => router.push(`/admin/sessions/${id}?tab=rimpilan`), 1200);
      }
      loadCount();
    } catch (err) {
      setError(err.message);
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      <div>
        ...
        <h1 className="mt-1 text-xl font-semibold tracking-tight">Upload Master Rimpilan</h1>
        <p className="mt-1 text-sm text-ink/60">
          Template sama seperti Data SAP (Material, Batch, Base Unit of Measure, Plant, Storage
          Location, Material Group, Qty) ditambah dua kolom: <strong>Nomor Rak</strong> dan{' '}
          <strong>Level</strong> (1–7, opsional — default 1 kalau kosong, bisa dikoreksi petugas saat input).
        </p>
      </div>
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="text-ink/50">
                ...
                <th className="pb-2 pr-3">Nomor Rak</th>
            ...
            <tbody>
              {parsedRows.slice(0, 8).map((r, i) => (
                <tr key={i} className="border-t border-line">
                  ...
                  <td className="py-1.5 pr-3 font-mono">{r.level}</td>
          ...
          {totalCount > 0 && (
            <div className="mt-4 rounded-lg border border-warn/30 bg-warn/5 p-4">
              <div className="mb-2 text-sm font-medium text-warn">
                Session ini sudah punya {totalCount} baris Master Rimpilan.
              </div>
