...
export default function RimpilanInputPage() {
  ...
  async function load() {
    ...
    const [{ data: sessionData }, racksData, materialsData] = await Promise.all([
      supabase.from('so_sessions').select('active_recount_round, recount_material_codes').eq('id', id).single(),
    ...
    if (sessionData) {
      ...
      const eligible = codes.length > 0 && typeof codes[0] === 'object'
        ? codes.filter((c) => c.source !== 'normal').map((c) => c.code)
        : [];
      setRecountCodes(eligible);
    }
    setLoading(false);
  }

  function getRowState(row) {
    return rowState[row.id] || { level: row.level || 1, normalQty: '', keterangan: [] };
  }

  function updateRow(rowId, patch) {
    setRowState((prev) => ({
      ...prev,
      [rowId]: { ...(prev[rowId] || { level: 1, normalQty: '', keterangan: [] }), ...patch },
    }));
  }

  // These three take the full `row` (not just its id) so the very first
  // interaction with a material — clicking "+ Tambah Keterangan Khusus"
  // before ever touching the Level dropdown — still seeds the default
  // Level from the master data (row.level) instead of silently falling
  // back to 1.
  ...
  function buildEntriesToSave(rows) {
    ...
    for (const row of rows) {
      ...
      const base = {
        ...
        level: Number(state.level) || row.level,
        recount_round: recountMode ? activeRecountRound : 0,
      };

      if (state.normalQty !== '' && state.normalQty !== undefined && state.normalQty !== null) {
                          ...
                          <div className="space-y-3 border-t border-line p-4">
                            {list.map((row) => (
                              <MaterialRowEditor
                                ...
                                state={getRowState(row)}
                                onChange={(patch) => updateRow(row.id, patch)}
                                onAddKeterangan={() => addKeterangan(row.id)}
                                onUpdateKeterangan={(ketId, patch) => updateKeterangan(row.id, ketId, patch)}
                                onRemoveKeterangan={(ketId) => removeKeterangan(row.id, ketId)}
                              />
                            ))}
                          </div>
                        )}
...
function MaterialRowEditor({ row, state, onChange, onAddKeterangan, onUpdateKeterangan, onRemoveKeterangan }) {
  return (
    <div className="rounded-lg border border-line p-3.5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select className="input-field" value={state.level} onChange={(e) => onChange({ level: Number(e.target.value) })}>
            {LEVELS.map((lv) => (
