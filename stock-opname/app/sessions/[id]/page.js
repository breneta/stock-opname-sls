  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <Link href="/" className="text-xs text-ink/50 hover:text-ink">← Semua Session</Link>
      ...
      {!stats.hasSapData ? (
        <div className="card border-amber/40 bg-amber/10 p-5">
          ...
          <Link href={`/sessions/${id}/upload`} className="btn-primary mt-4 inline-flex">
            Upload Data SAP
          </Link>
        </div>
      ) : (
        <>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <ActionLink
              ...
              title="Mulai / Lanjut Input"
              desc="Input hasil hitung fisik di lapangan"
              primary
            />
            <ActionLink
              href={`/sessions/${id}/reconciliation`}
              title="Rekonsiliasi"
...
function ActionLink({ href, title, desc, primary }) {
  return (
    <Link
      href={href}
      className={`card block p-4 transition hover:shadow-md ${primary ? 'border-slate-850/30 bg-slate-850 text-white hover:brightness-110' : 'hover:border-slate-850/30'}`}
    >
      <div className="font-medium">{title}</div>
      <div className={`mt-0.5 text-sm ${primary ? 'text-white/70' : 'text-ink/50'}`}>{desc}</div>
    </Link>
  );
}
