...
export default function InputPage() {
  ...
  // Rak grouping for recount mode — rak sangat krusial buat petugas jalan
  // fisik per rak, bukan loncat-loncat per kode material. Nomor rak bukan
  // master data buat Normal SO (cuma yang pernah diketik petugas sebelumnya),
  // jadi ini dihitung dari histori so_entries + rimpilan_entries, sama
  // persis logikanya kayak Rak Selisih page — satu material bisa muncul di
  // lebih dari satu rak kalau memang pernah dihitung di rak berbeda.
  ...
  // histori entry (bukan master data), lalu kelompokkan material per rak.
  // Material yang belum pernah dihitung sama sekali masuk ke bucket
  ...
  async function loadRecountRakGroups(codes) {
    const [{ data: normalEntries }, { data: rimpilanEntries }] = await Promise.all([
      supabase
        .from('so_entries')
        .select('material_code, material_description, nomor_rak')
        .eq('session_id', id)
        .is('voided_at', null)
    ...
    const byRak = new Map(); // rak -> Map<material_code, {material, material_description}>
    const addEntry = (e) => {
      ...
      if (!group.has(e.material_code)) {
        group.set(e.material_code, { material: e.material_code, material_description: e.material_description });
      }
    };
    ...
    if (unseenCodes.length > 0) {
      byRak.set(
        ...
        new Map(unseenCodes.map((c) => [c, { material: c, material_description: '' }]))
      );
    }
  ...
  return (
    <div className="mx-auto max-w-lg space-y-4">
                    ...
                    <div className="space-y-1 border-t border-line px-2 py-2">
                      {g.materials.map((m) => (
                        <button
                          key={m.material}
                          type="button"
                          className="block w-full rounded-md px-2 py-1.5 text-left text-xs hover:bg-paper"
                          onClick={() => {
                            setMaterialCode(m.material);
                            setNomorRak(g.rak.startsWith('(') ? '' : g.rak);
                            runMaterialLookup(m.material);
                            materialInputRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                          }}
                        >
                          <span className="font-mono font-medium text-ink">{m.material}</span>
                          {m.material_description && (
                            <span className="ml-1.5 text-ink/50">{m.material_description}</span>
                          )}
                        </button>
                      ))}
                    </div>
      ...
      <form onSubmit={handleSave} className="card space-y-4 p-5">
        ...
        {recountMode && showDetails && normalRecountMaterials.includes(materialCode.trim()) && (
          <div className="rounded-lg border border-warn/30 bg-warn/5 p-3.5 text-sm">
            <div className="font-medium text-ink/70">Entry lama material ini</div>
            ...
            {loadingHistory ? (
              <div className="mt-2 text-xs text-ink/40">Memuat riwayat...</div>
            ) : materialHistory.length === 0 ? (
              <div className="mt-2 text-xs text-ink/40">Belum ada entry sebelumnya untuk material ini.</div>
            ) : (
              <div className="mt-2.5 space-y-2">
                {materialHistory.map((h) => {
                  const hKey = `${h.source}:${h.id}`;
                  return (
                    <div key={hKey} className="rounded-lg border border-line bg-white p-2.5 text-xs">
                      ...
                      {voidingHistoryKey === hKey && (
                        <div className="mt-2 space-y-1.5 border-t border-line pt-2">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button type="button" onClick={() => setVoidingHistoryKey(null)} className="btn-ghost text-xs">
                              Batal
                            </button>
                          </div>
                        </div>
