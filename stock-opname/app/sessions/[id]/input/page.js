...
export default function InputPage() {
  ...
  // yang salah sama sekali — bukan cuma field yang keliru. Nggak ada
  // gunanya prefill balik ke material code yang SAMA (yang salah), jadi
  // cuma void + kasih arahan ke mana harus input entry yang benar.
  async function confirmRowMaterialCodeVoid(code, entry) {
    const ok = await voidRowHistoryEntry(code, entry);
    if (!ok) return;

    setRakInfo(
      `Entry lama "${entry.material_code}" sudah dibatalkan. Cari material code yang BENAR di rak ini atau rak ` +
        `lain (termasuk bucket "(belum pernah dihitung)"), lalu isi Qty di situ. Kalau material yang benar belum ` +
        `muncul di daftar, berarti belum di-approve untuk recount — minta Admin tambahkan dulu.`
    );
