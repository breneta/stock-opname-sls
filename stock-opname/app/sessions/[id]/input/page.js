...
export default function InputPage() {
  ...
  const [sessionPlant, setSessionPlant] = useState(null);
  const materialInputRef = useRef(null);
  const nomorRakRef = useRef(null);

  const [materialCode, setMaterialCode] = useState('');
  const [lookupState, setLookupState] = useState('idle'); // idle | searching | found | notfound
  const [source, setSource] = useState('sap'); // sap | rimpilan
  const [batchOptions, setBatchOptions] = useState([]); // sap rows sharing the material code
  const [selectedRow, setSelectedRow] = useState(null); // chosen sap row
  const [rimpilanRow, setRimpilanRow] = useState(null); // matched mr_materials row
  ...
  useEffect(() => {
    ...
    setPetugas(saved);
    supabase.from('so_sessions').select('plant').eq('id', id).single().then(({ data }) => {
      setSessionPlant(data?.plant || null);
    });
  }, [id, router]);
  ...
  async function handleMaterialKeyDown(e) {
    ...
    if (sapRows && sapRows.length > 0) {
      const distinctBatches = [...new Map(sapRows.map((r) => [r.batch, r])).values()];
      setSource('sap');
    ...
    // Not in SAP — try Master Rimpilan for this session's RDC.
    if (sessionPlant) {
      const { data: rimpilanData, error: rimpilanError } = await supabase
        .from('mr_materials')
        .select('*')
        .eq('plant', sessionPlant)
        .ilike('kode_material', code)
        .maybeSingle();

      if (!rimpilanError && rimpilanData) {
        setSource('rimpilan');
        setRimpilanRow(rimpilanData);
        setBatchOptions([]);
        setSelectedRow(null);
        setLookupState('found');
        setNomorRak(rimpilanData.nomor_rak || '');
        focusNomorRak();
        return;
      }
    }
  ...
  async function handleSave(e) {
    ...
    if (source === 'sap' && batchOptions.length > 1 && !selectedRow && statusSap !== 'tidak_ada_di_sap') {
    ...
    setSaving(true);
    const record = source === 'rimpilan' && rimpilanRow
      ? {
          session_id: id,
          petugas_nama: petugas,
          material_code: materialCode.trim(),
          material_description: rimpilanRow.nama_material,
          batch: null,
          plant: sessionPlant,
  ...
  const showBatchPicker = source === 'sap' && lookupState === 'found' && batchOptions.length > 1 && !selectedRow;
  const showSapDetails = source === 'sap' && selectedRow;
  const showRimpilanDetails = source === 'rimpilan' && rimpilanRow;
  const showNotFoundDetails = source === 'sap' && !selectedRow && statusSap === 'tidak_ada_di_sap' && !rimpilanRow;
    ...
    <div className="mx-auto max-w-lg space-y-4">
      ...
      {lastSaved && (
        <div className="card border-good/30 bg-good/5 p-3 text-sm text-good">
          ...
          {lastSaved.source === 'rimpilan' && <span className="ml-1 text-ink/50">(Rimpilan)</span>}
        </div>
      )}
      ...
      <form onSubmit={handleSave} className="card space-y-4 p-5">
        <div>
          ...
          {lookupState === 'searching' && (
            <div className="mt-1.5 text-xs text-ink/50">Mencari di Data SAP & Master Rimpilan...</div>
          )}
        </div>
        ...
        {showSapDetails && (
          <div className="rounded-lg bg-paper p-3.5 text-sm">
            <div className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-ink/40">Sumber: Data SAP</div>
        ...
        {showRimpilanDetails && (
          <div className="rounded-lg bg-teal/10 p-3.5 text-sm">
            <div className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-teal-700">Sumber: Master Rimpilan</div>
            <div className="grid grid-cols-2 gap-y-1.5">
              <span className="text-ink/50">Material</span>
              <span className="font-mono">{materialCode}</span>
              <span className="text-ink/50">Nama</span>
              <span>{rimpilanRow.nama_material}</span>
              <span className="text-ink/50">Nomor Rak (Master)</span>
              <span className="font-mono">{rimpilanRow.nomor_rak || '—'}</span>
              <span className="text-ink/50">Satuan</span>
              <span>{rimpilanRow.satuan}</span>
            </div>
          </div>
        )}

        {showNotFoundDetails && (
          <div className="rounded-lg bg-paper p-3.5 text-sm text-warn">
            Material "{materialCode}" — Tidak ditemukan di SAP maupun Master Rimpilan. Data akan tetap disimpan.
          </div>
        )}
      ...
      {showNotFoundModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/40 p-4 sm:items-center">
          <div className="w-full max-w-sm rounded-xl bg-white p-5 shadow-lg">
            <div className="font-medium">Material tidak ditemukan di SAP maupun Master Rimpilan.</div>
            <p className="mt-2 text-sm text-ink/60">Kemungkinan penyebab:</p>
            <ul className="mt-1 list-inside list-disc text-sm text-ink/60">
              <li>Material baru belum ada pada Data SAP / Master Rimpilan.</li>
