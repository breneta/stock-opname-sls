...
export default function InputPage() {
  ...
  const revisionFormRef = useRef(null);
  const revisionToInputRef = useRef(null);
  ...
  // Material yang di-void ORANG LAIN (Admin lewat Kelola Entry, atau
  // petugas lain) dan sekarang belum punya entry aktif — dipakai buat kartu
  // "Perlu Direvisi" biar kelihatan langsung, nggak perlu dicari manual.
  const [adminVoidedNeedsRevision, setAdminVoidedNeedsRevision] = useState([]);
  ...
  // Info non-error (biru/netral), dipakai buat arahan setelah "Salah
  // Material Code" — beda dari rakError yang selalu merah/blocking.
  const [rakInfo, setRakInfo] = useState(null);
  ...
  // Same pattern, buat combobox koreksi material code ("Salah Material
  // Code" flow) — suggestion-nya dibatasi ke material yang sudah
  // di-approve untuk recount (normalRecountMaterials).
  const [materialOverrideSuggestFor, setMaterialOverrideSuggestFor] = useState(null);
  // Panel "Revisi Kode Material" — TERTUTUP secara default, cuma dibuka
  // kalau dipicu (klik "Salah kode?" di kartu material / kartu "Perlu
  // Direvisi"), atau dibuka manual lewat link kalau petugas mau mulai dari
  // sini langsung. Isi kode yang salah + kode yang benar sekaligus.
  const [revisionPanelOpen, setRevisionPanelOpen] = useState(false);
  const [revisionFromCode, setRevisionFromCode] = useState('');
  const [revisionToCode, setRevisionToCode] = useState('');
  const [revisionReason, setRevisionReason] = useState('');
  const [revisionFromSuggestOpen, setRevisionFromSuggestOpen] = useState(false);
  const [revisionToSuggestOpen, setRevisionToSuggestOpen] = useState(false);
  ...
  async function loadRecountRakGroups(codes) {
    const [{ data: normalEntries }, { data: rimpilanEntries }, { data: voidedEntries }] = await Promise.all([
      supabase
        .from('so_entries')
      ...
      supabase
        ...
        .in('material_code', codes)
        .order('created_at', { ascending: false }),
      // Entry yang dibatalkan ORANG LAIN (Admin lewat Kelola Entry, atau
      // petugas lain) — kalau materialnya sekarang nggak punya entry aktif
      // sama sekali, berarti perlu di-input ulang. Dipakai buat kartu
      // "Perlu Direvisi" di bawah biar petugas nggak perlu nyari sendiri.
      supabase
        .from('so_entries')
        .select('id, material_code, material_description, batch, level, qty_fisik, keterangan_khusus, keterangan_catatan, nomor_rak, voided_at, voided_by, void_reason')
        .eq('session_id', id)
    ...
    // Material yang punya riwayat dibatalkan TAPI sekarang beneran nggak
    // punya entry aktif (unseenCodes) — jadi yakin belum ada yang input
    // ulang. voidedEntries sudah urut terbaru dulu, jadi ambil satu per
    // material_code = entry terakhir yang dibatalkan.
    //
    // Sengaja TIDAK difilter berdasarkan "voided_by beda dari petugas ini" —
    // nama voided_by dan nama petugas bisa saja kebetulan sama (mis. orang
    // yang sama pegang peran Admin & petugas), jadi perbandingan nama itu
    // rapuh dan bisa nyembunyiin item yang harusnya tetap muncul. Kalaupun
    // ini muncul dobel dengan pengalaman self-void (baris yang materialnya
    // sedang dikoreksi di accordion), itu nggak masalah — cuma pengingat
    // tambahan, bukan aksi ganda.
    const unseenSet = new Set(unseenCodes);
    const revisionByMaterial = new Map();
    for (const v of voidedEntries || []) {
      if (!unseenSet.has(v.material_code)) continue;
      if (!revisionByMaterial.has(v.material_code)) revisionByMaterial.set(v.material_code, v);
    }
    setAdminVoidedNeedsRevision([...revisionByMaterial.values()]);
  }
  ...
  // Void inti dipakai dua cara berbeda (lihat dua fungsi di bawah): guard +
  // update Supabase-nya sama persis, cuma beda apa yang terjadi SETELAH
  // void-nya berhasil. Return true/false biar caller tahu lanjut atau stop.
  async function voidRowHistoryEntry(code, entry) {
    if (!recountMode || !normalRecountMaterials.includes(entry.material_code)) {
  ...
  // "Batalkan & Isi Ulang": dipakai kalau yang salah itu qty/level/batch,
  // material code-nya sendiri sudah benar. Begitu entry lama di-void, baris
  // rak untuk material ini langsung diisi ulang dari data entry yang
  // dibatalkan (batch/level/qty atau keterangan khusus) supaya petugas
  // tinggal koreksi field yang salah, bukan ngetik ulang dari nol — lalu
  // tetap harus tekan "Simpan Rak" seperti biasa untuk benar-benar
  // menyimpan baris baru itu (jadi belum ada perubahan data sebelum
  // disimpan).
  async function confirmRowHistoryVoid(code, entry) {
    const ok = await voidRowHistoryEntry(code, entry);
    if (!ok) return;
  ...
  // "Salah Material Code": dipakai kalau entry lama nyasar ke material code
  // yang salah sama sekali — bukan cuma field yang keliru. Void entry lama,
  // lalu baris ini masuk "mode koreksi": label material code berubah jadi
  // field yang bisa diketik/dicari ulang (materialOverride), sementara
  // Level & Qty Fisik dipertahankan (fisiknya tetap sama, cuma kodenya yang
  // salah) dan Batch dikosongkan lagi karena batch itu spesifik per
  // material, harus di-resolve ulang untuk material yang benar.
  async function confirmRowMaterialCodeVoid(code, entry) {
    const ok = await voidRowHistoryEntry(code, entry);
    if (!ok) return;
    ...
    updateRakRow(code, {
      ...
      batch: undefined,
      level: entry.level ?? currentRow.level,
      qtyFisik: entry.keterangan_khusus ? currentRow.qtyFisik : String(entry.qty_fisik ?? ''),
      keterangan: entry.keterangan_khusus
        ? [
            ...currentRow.keterangan,
            { id: nextKeteranganId(), jenis: entry.keterangan_khusus, qty: String(entry.qty_fisik ?? ''), catatan: entry.keterangan_catatan || '' },
          ]
    ...
    setRakInfo(
      `Entry lama "${entry.material_code}" sudah dibatalkan. Ketik/cari material code yang BENAR di field yang ` +
        `muncul di atas, lalu cek lagi Batch-nya sebelum Simpan Rak.`
    );
    toggleRowHistory(code);
    setOpenHistoryFor(code);
    loadRecountRakGroups(normalRecountMaterials);
  }

  // Dipanggil dari mana aja (kartu "Perlu Direvisi", baris di accordion) —
  // tinggal pencet material code yang salah, langsung keisi ke form Revisi
  // di atas dan fokus pindah ke "Kode yang BENAR" — petugas tinggal ketik
  // kode yang benar + alasan, maksimal 1-2 klik dari kartu material.
  function pickAsWrongCode(code) {
    setRevisionPanelOpen(true);
    setRevisionFromCode(code);
    setRevisionFromSuggestOpen(false);
    setTimeout(() => {
      revisionFormRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      revisionToInputRef.current?.focus();
    }, 50);
  }

  // Buka rak yang berisi material tertentu + isi kotak pencarian dengan kode
  // itu, biar kartunya kesorot (isSearchMatch) — dipakai setelah aksi revisi
  // supaya petugas langsung diarahkan ke kartu yang perlu diisi.
  function revealMaterialInAccordion(code) {
    setRakSearch(code);
    const group = recountRakGroups.find((g) => g.materials.some((mm) => mm.material === code));
    setOpenRak(group ? group.rak : '(belum pernah dihitung)');
  }

  // Dipanggil dari kartu "Perlu Direvisi" — material ini sudah dibatalkan
  // ORANG LAIN (Admin/petugas lain) dan belum ada yang input ulang. Karena
  // dia nggak punya entry aktif sama sekali, dia pasti ada di bucket
  // "(belum pernah dihitung)" — langsung buka bucket itu, prefill baris-nya
  // dari data entry lama yang dibatalkan (biar tinggal cek/koreksi), dan
  // sorot materialnya lewat search yang sudah ada.
  function jumpToRevision(v) {
    updateRakRow(v.material_code, {
      batch: v.batch || undefined,
      level: v.level ?? '',
      qtyFisik: v.keterangan_khusus ? '' : String(v.qty_fisik ?? ''),
      keterangan: v.keterangan_khusus
        ? [{ id: nextKeteranganId(), jenis: v.keterangan_khusus, qty: String(v.qty_fisik ?? ''), catatan: v.keterangan_catatan || '' }]
        : [],
    });
    revealMaterialInAccordion(v.material_code);
  }

  // Form "Revisi Kode Material" — jalur PALING LANGSUNG buat benerin salah
  // ketik material code: petugas isi kode yang salah + kode yang benar
  // sekaligus di satu form, nggak perlu cari-cari tombol di riwayat baris.
  //
  // Entry TERBARU milik kode yang salah dicari TANPA peduli statusnya:
  //   - Kalau masih AKTIF (belum di-void) → dibatalkan sekarang, alasan dari
  //     form ini yang dipakai.
  //   - Kalau SUDAH di-void sebelumnya (misal material ini datang dari
  //     kartu "Perlu Direvisi" — sudah dibatalkan Admin) → nggak dibatalkan
  //     dobel, tinggal datanya dipakai buat prefill.
  // Kedua kasus berakhir sama: baris material yang BENAR di-prefill dari
  // data entry itu dan dibuka di accordion — tinggal cek Batch lalu Simpan
  // Rak.
  async function submitCodeRevision() {
    setRakError(null);
    const fromCode = revisionFromCode.trim();
    const toCode = revisionToCode.trim();
    const reason = revisionReason.trim();
    if (!fromCode || !toCode) {
      setRakError('Isi kode yang salah dan kode yang benar dulu.');
      return;
    }
    if (fromCode === toCode) {
      setRakError('Kode yang salah dan kode yang benar tidak boleh sama.');
      return;
    }
    if (!normalRecountMaterials.includes(fromCode)) {
      setRakError(`${fromCode}: material ini tidak sedang di-recount.`);
      return;
    }
    if (!normalRecountMaterials.includes(toCode)) {
      setRakError(`${toCode}: material ini belum di-approve untuk recount. Minta Admin tambahkan dulu.`);
      return;
    }

    setSavingRevision(true);
    const [{ data: normalRows }, { data: rimpilanRows }] = await Promise.all([
      supabase
        .from('so_entries')
        .select('*')
        .eq('session_id', id)
        .eq('material_code', fromCode)
        .order('created_at', { ascending: false })
        .limit(1),
      supabase
        .from('rimpilan_entries')
    ...
    const entry = [
      ...(normalRows || []).map((e) => ({ ...e, source: 'normal' })),
      ...(rimpilanRows || []).map((e) => ({ ...e, source: 'rimpilan' })),
    ].sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0];

    if (!entry) {
      setSavingRevision(false);
      setRakError(`${fromCode}: belum ada entry sama sekali untuk kode ini.`);
      return;
    }

    if (!entry.voided_at) {
      // Masih aktif — ini yang mau dibatalkan sekarang, alasan wajib diisi.
      if (!reason) {
        setSavingRevision(false);
        setRakError('Alasan revisi wajib diisi.');
        return;
      }
      const table = entry.source === 'normal' ? 'so_entries' : 'rimpilan_entries';
      const { error: updateError } = await supabase
    ...
    updateRakRow(toCode, {
      batch: undefined,
      level: entry.level ?? '',
      qtyFisik: entry.keterangan_khusus ? '' : String(entry.qty_fisik ?? ''),
      keterangan: entry.keterangan_khusus
        ? [{ id: nextKeteranganId(), jenis: entry.keterangan_khusus, qty: String(entry.qty_fisik ?? ''), catatan: entry.keterangan_catatan || '' }]
        : [],
    });
    ...
    setRakInfo(
      entry.voided_at
        ? `Data lama "${fromCode}" dipakai buat "${toCode}" — kartu-nya sudah kebuka di bawah, cek Batch-nya lalu Simpan Rak.`
        : `Entry "${fromCode}" sudah dibatalkan. Kartu "${toCode}" sudah kebuka di bawah — cek Batch-nya lalu Simpan Rak.`
    );
    setRevisionFromCode('');
    setRevisionToCode('');
    setRevisionReason('');
    setRevisionPanelOpen(false);
    revealMaterialInAccordion(toCode);
    loadRecountRakGroups(normalRecountMaterials);
  }

  function getRakRow(code) {
    return rakRowState[code] || { batch: undefined, level: '', qtyFisik: '', keterangan: [], savedAt: null, materialOverride: undefined };
  }
  ...
  // Material code efektif buat satu baris — normalnya sama dengan m.material,
  // tapi kalau lagi mode koreksi "Salah Material Code" (materialOverride
  // terisi), pakai kode yang baru diketik itu.
  function effectiveMaterialCode(m) {
    const row = getRakRow(m.material);
    return row.materialOverride !== undefined && row.materialOverride.trim() ? row.materialOverride.trim() : m.material;
  }
  ...
  // lastBatch hint hanya dipakai kalau material code-nya belum dikoreksi —
  // batch terakhir punya material yang SALAH nggak relevan buat material
  // yang benar.
  function resolveRowBatch(m, code) {
    const effectiveCode = code || m.material;
    const options = recountBatchOptions[effectiveCode] || [];
    const row = getRakRow(m.material);
    if (row.batch) return options.find((o) => o.batch === row.batch) || null;
    if (options.length === 1) return options[0];
    if (effectiveCode === m.material && m.lastBatch && options.some((o) => o.batch === m.lastBatch)) {
  ...
  async function handleSaveRak(rak, materials) {
    ...
    for (const m of materials) {
      ...
      // Kalau baris ini lagi dikoreksi ("Salah Material Code"), material
      // code yang benar-benar dipakai buat simpan adalah yang diketik ulang
      // (materialOverride), bukan m.material (kode lama yang salah).
      const effectiveCode = effectiveMaterialCode(m);
      if (row.materialOverride !== undefined && !row.materialOverride.trim()) {
        setRakError(`${m.material}: material code yang benar wajib diisi dulu (atau tekan "Batal, pakai material code semula").`);
        return;
      }
      if (!normalRecountMaterials.includes(effectiveCode)) {
        setRakError(`${effectiveCode}: material ini belum di-approve untuk recount. Minta Admin tambahkan dulu.`);
        return;
      }

      if (!hasQty) {
        setRakError(`${effectiveCode}: Qty Fisik wajib diisi.`);
        return;
      }
      const qtyNum = Number(row.qtyFisik);
      if (Number.isNaN(qtyNum) || qtyNum < 0) {
        setRakError(`${effectiveCode}: Qty Fisik harus angka >= 0.`);
        return;
      }
      if (!row.level) {
        setRakError(`${effectiveCode}: Level wajib dipilih.`);
        return;
      }
      const resolvedBatch = resolveRowBatch(m, effectiveCode);
      ...
      if (!typedBatch) {
        setRakError(`${effectiveCode}: Batch wajib diisi.`);
        return;
      }
      ...
      const base = {
        ...
        material_code: effectiveCode,
        material_description: resolvedBatch?.material_description || (effectiveCode === m.material ? m.material_description : null) || null,
      ...
      for (const k of row.keterangan) {
        ...
        if (Number.isNaN(kQty) || kQty < 0) {
          setRakError(`${effectiveCode} (${k.jenis}): Qty harus angka >= 0.`);
          return;
        }
    ...
    // Refresh grouping + kartu "Perlu Direvisi" — material yang baru
    // disimpan (termasuk hasil koreksi material code) harus lepas dari
    // daftar revisi dan pindah ke rak yang benar.
    loadRecountRakGroups(normalRecountMaterials);
  }
  ...
  // Lookup material+description buat suggestion combobox koreksi material
  // code — digabung dari semua rak group yang sudah kemuat (recountRakGroups
  // sudah mencakup SEMUA material approved-recount, termasuk yang belum
  // pernah dihitung, jadi ini otomatis lengkap).
  const recountMaterialInfoByCode = {};
  recountRakGroups.forEach((g) => g.materials.forEach((mm) => { recountMaterialInfoByCode[mm.material] = mm; }));
  ...
  function renderRakAccordion(groups) {
    ...
    // Pencarian ini nyari di DUA hal sekaligus: nama rak, ATAU material
    // code/description di dalam rak itu — jadi kalau lagi benerin "Salah
    // Material Code", petugas tinggal ketik material code yang benar buat
    // langsung nemu rak-nya, nggak perlu buka satu-satu rak yang ada.
    const groupMatches = (g) =>
      g.rak.toLowerCase().includes(term) ||
      g.materials.some(
        (m) => m.material.toLowerCase().includes(term) || (m.material_description || '').toLowerCase().includes(term)
      );
    const filteredGroups = term ? groups.filter(groupMatches) : groups;
    return (
      <div className="space-y-1.5">
        <input
          ...
          onChange={(e) => {
            ...
            setRakSearch(val);
            // Auto-expand kalau ketikan ini cuma cocok di satu rak — biar
            // nggak perlu tap lagi buat buka rak-nya.
            const t = val.trim().toLowerCase();
            if (t) {
              const matches = groups.filter(
                (g) =>
                  g.rak.toLowerCase().includes(t) ||
                  g.materials.some(
                    (m) => m.material.toLowerCase().includes(t) || (m.material_description || '').toLowerCase().includes(t)
                  )
              );
              if (matches.length === 1) setOpenRak(matches[0].rak);
            }
          }}
          placeholder="Cari rak atau material code..."
        />
        {filteredGroups.length === 0 && (
          <div className="p-3 text-center text-xs text-ink/40">Tidak ada rak atau material yang cocok.</div>
        )}
              ...
              {isOpen && (
                <div className="space-y-2.5 border-t border-line p-2.5">
                  {g.materials.map((m) => {
                    const row = getRakRow(m.material);
                    const effectiveCode = effectiveMaterialCode(m);
                    const batchOptions = recountBatchOptions[effectiveCode] || [];
                    const resolvedBatch = resolveRowBatch(m, effectiveCode);
                    const isSearchMatch =
                      term &&
                      (m.material.toLowerCase().includes(term) || (m.material_description || '').toLowerCase().includes(term));
                    return (
                      <div
                        key={m.material}
                        className={`rounded-lg border p-2.5 text-sm ${
                          isSearchMatch ? 'border-amber ring-2 ring-amber/40' : row.savedAt ? 'border-good/30 bg-good/5' : 'border-line bg-white'
                        }`}
                      >
                        <div className="flex items-start justify-between gap-2">
                          {row.materialOverride !== undefined ? (
                            <div className="relative flex-1">
                              <label className="label-field text-xs">Material Code (koreksi dari {m.material})</label>
                              <input
                                className="input-field font-mono"
                                value={row.materialOverride}
                                onChange={(e) => updateRakRow(m.material, { materialOverride: e.target.value })}
                                onFocus={() => setMaterialOverrideSuggestFor(m.material)}
                                onBlur={() => setTimeout(() => setMaterialOverrideSuggestFor((f) => (f === m.material ? null : f)), 150)}
                                placeholder="Ketik material code yang benar..."
                                autoComplete="off"
                                autoFocus
                              />
                              {materialOverrideSuggestFor === m.material && (() => {
                                const typed = row.materialOverride.trim().toLowerCase();
                                const pool = Object.values(recountMaterialInfoByCode).filter((mm) => mm.material !== m.material);
                                const matches = (typed
                                  ? pool.filter((mm) => mm.material.toLowerCase().includes(typed) || (mm.material_description || '').toLowerCase().includes(typed))
                                  : pool
                                ).slice(0, 20);
                                    ...
                                    {matches.map((mm) => (
                                      <li key={mm.material}>
                                        <button
                                          ...
                                          onMouseDown={(e) => {
                                            e.preventDefault();
                                            updateRakRow(m.material, { materialOverride: mm.material });
                                            setMaterialOverrideSuggestFor(null);
                                          }}
                                        >
                                          <span className="font-mono font-medium">{mm.material}</span>
                                          {mm.material_description && <span className="ml-1.5 text-ink/50">— {mm.material_description}</span>}
                                        </button>
                                      </li>
                                    ))}
                                  </ul>
                              ...
                              {row.materialOverride.trim() && !normalRecountMaterials.includes(row.materialOverride.trim()) && (
                                <div className="mt-1 text-xs text-warn">
                                  "{row.materialOverride.trim()}" belum ada di daftar approved recount — minta Admin tambahkan dulu sebelum bisa disimpan.
                                </div>
                              )}
                              <button
                                type="button"
                                onClick={() => updateRakRow(m.material, { materialOverride: undefined })}
                                className="mt-1 text-xs text-ink/40 hover:text-ink hover:underline"
                              >
                                Batal, pakai material code semula ({m.material})
                              </button>
                            </div>
                          ) : (
                            <div>
                              <button
                                type="button"
                                onClick={() => pickAsWrongCode(m.material)}
                                className="font-mono text-sm font-medium text-ink hover:underline"
                                title="Pencet kalau kode ini yang salah — otomatis keisi ke form Revisi di atas"
                              >
                                {m.material}
                              </button>
                              ...
                              <button
                                type="button"
                                onClick={() => pickAsWrongCode(m.material)}
                                className="mt-0.5 block text-xs text-ink/40 hover:text-ink hover:underline"
                              >
                                Salah kode? →
                              </button>
                            </div>
                                        ...
                                        <div className="mt-2 space-y-1.5 border-t border-line pt-2">
                                          ...
                                          <div className="flex flex-wrap gap-2">
                                            ...
                                            <button
                                              type="button"
                                              disabled={savingHistoryVoid}
                                              onClick={() => confirmRowMaterialCodeVoid(m.material, h)}
                                              className="inline-flex items-center justify-center rounded-lg border border-bad/40 px-3 py-1.5 text-xs font-medium text-bad transition hover:bg-bad/5 active:scale-[0.98] disabled:opacity-40 disabled:pointer-events-none"
                                            >
                                              Salah Material Code →
                                            </button>
                  ...
                  {rakInfo && <div className="rounded-lg border border-line bg-paper p-2 text-xs text-ink/70">{rakInfo}</div>}
        ...
        <div className="card border-warn/40 bg-warn/10 p-3.5 text-sm">
          ...
          {!revisionPanelOpen && (
            <button
              ...
              className="mt-3 text-xs font-medium text-slate-850 hover:underline"
            >
              Salah ketik material code? Revisi di sini →
            </button>
          )}

          {revisionPanelOpen && (
          <div ref={revisionFormRef} className="mt-3 rounded-lg border border-line bg-white p-2.5 text-xs">
            <div className="flex items-center justify-between">
              <div className="font-medium text-ink/70">Revisi Kode Material</div>
              <button
                type="button"
                onClick={() => {
                  setRevisionPanelOpen(false);
            ...
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
              <div className="relative">
                <label className="label-field text-xs">Kode yang SALAH</label>
                <input
                  className="input-field font-mono"
                  value={revisionFromCode}
                  onChange={(e) => setRevisionFromCode(e.target.value)}
                  onFocus={() => setRevisionFromSuggestOpen(true)}
                  onBlur={() => setTimeout(() => setRevisionFromSuggestOpen(false), 150)}
                  placeholder="Ketik/cari kode yang sudah terlanjur diinput..."
                  autoComplete="off"
                />
                {revisionFromSuggestOpen && (() => {
                  const typed = revisionFromCode.trim().toLowerCase();
                  const pool = Object.values(recountMaterialInfoByCode);
                  const matches = (typed
                    ? pool.filter((mm) => mm.material.toLowerCase().includes(typed) || (mm.material_description || '').toLowerCase().includes(typed))
                    : pool
                  ).slice(0, 20);
                      ...
                      {matches.map((mm) => (
                        <li key={mm.material}>
                          <button
                            ...
                            onMouseDown={(e) => {
                              e.preventDefault();
                              setRevisionFromCode(mm.material);
                              setRevisionFromSuggestOpen(false);
                            }}
                          >
                            <span className="font-mono font-medium">{mm.material}</span>
              ...
              <div className="relative">
                <label className="label-field text-xs">Kode yang BENAR</label>
                <input
                  ref={revisionToInputRef}
                  className="input-field font-mono"
                  value={revisionToCode}
                  onChange={(e) => setRevisionToCode(e.target.value)}
                  onFocus={() => setRevisionToSuggestOpen(true)}
                  onBlur={() => setTimeout(() => setRevisionToSuggestOpen(false), 150)}
                  placeholder="Ketik/cari kode yang seharusnya..."
                  autoComplete="off"
                />
                {revisionToSuggestOpen && (() => {
                  const typed = revisionToCode.trim().toLowerCase();
                  const pool = Object.values(recountMaterialInfoByCode).filter((mm) => mm.material !== revisionFromCode.trim());
                  const matches = (typed
                    ? pool.filter((mm) => mm.material.toLowerCase().includes(typed) || (mm.material_description || '').toLowerCase().includes(typed))
                    : pool
                  ).slice(0, 20);
                      ...
                      {matches.map((mm) => (
                        <li key={mm.material}>
                          <button
                            ...
                            onMouseDown={(e) => {
                              e.preventDefault();
                              setRevisionToCode(mm.material);
                              setRevisionToSuggestOpen(false);
                            }}
                          >
                            <span className="font-mono font-medium">{mm.material}</span>
                            {mm.material_description && <span className="ml-1.5 text-ink/50">— {mm.material_description}</span>}
            ...
            <textarea
              ...
              placeholder="Alasan (kalau kode lama masih aktif): contoh salah ketik, seharusnya ABGTB2201"
            />
            <button
              type="button"
              disabled={savingRevision}
              onClick={submitCodeRevision}
              className="btn-amber mt-2 w-full py-2 text-xs disabled:opacity-40 disabled:pointer-events-none"
            >
              {savingRevision ? 'Menyimpan...' : 'Revisi →'}
            </button>
            <p className="mt-1.5 text-ink/40">
              Kalau kode yang salah masih aktif, otomatis dibatalkan dulu. Kalau sudah dibatalkan sebelumnya
              (misal dari kartu "Perlu Direvisi"), datanya langsung dipakai. Kartu kode yang benar kebuka
              otomatis buat diisi Batch/Qty-nya.
            </p>
          </div>
          )}

          {adminVoidedNeedsRevision.length > 0 && (
            <div className="mt-3 rounded-lg border border-bad/30 bg-white p-2.5 text-xs">
              <div className="font-medium text-bad">
                {adminVoidedNeedsRevision.length} material perlu direvisi — entry lamanya dibatalkan, belum ada yang input ulang:
              </div>
              <div className="mt-2 space-y-2">
                {adminVoidedNeedsRevision.map((v) => (
                  <div key={v.material_code} className="rounded-lg border border-line bg-paper p-2">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div>
                        <div className="font-mono text-sm font-medium text-ink">{v.material_code}</div>
                        {v.material_description && <div className="text-ink/50">{v.material_description}</div>}
                        <div className="mt-1 text-ink/60">
                          Data lama: Batch {v.batch || '-'} · Rak {v.nomor_rak || '-'} · Lv {v.level ?? '-'}
                          {v.keterangan_khusus ? ` · ${v.keterangan_khusus} (Qty ${v.qty_fisik})` : ` · Qty ${v.qty_fisik}`}
                        </div>
                        <div className="mt-0.5 text-ink/40">
                          Dibatalkan {v.voided_by} · {new Date(v.voided_at).toLocaleString('id-ID')}
                          {v.void_reason ? ` — ${v.void_reason}` : ''}
                        </div>
                        <button
                          type="button"
                          onClick={() => pickAsWrongCode(v.material_code)}
                          className="mt-1 text-ink/40 hover:text-ink hover:underline"
                        >
                          Salah kode? →
                        </button>
                      </div>
                      <button
                        type="button"
                        onClick={() => jumpToRevision(v)}
                        className="btn-amber shrink-0 px-3 py-1.5 text-xs"
                      >
                        Input Ulang →
                      </button>
                    </div>
