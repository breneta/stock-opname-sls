...
export default function InputPage() {
  ...
  async function loadRecountRakGroups(codes) {
    ...
    // Material yang: (a) dibatalkan orang lain (bukan petugas ini sendiri —
    // self-void sudah dapat prefill instan, nggak perlu masuk sini juga),
    // dan (b) sekarang beneran nggak punya entry aktif (unseenCodes) — jadi
    // yakin belum ada yang input ulang. voidedEntries sudah urut terbaru
    // dulu, jadi ambil satu per material_code = entry terakhir yang
    // dibatalkan.
    // Baca petugas langsung dari localStorage (bukan closure state `petugas`)
    // supaya nggak stale kalau fungsi ini kepanggil dari efek mount sebelum
    // React sempat re-render setelah setPetugas.
    const currentPetugas = localStorage.getItem(`so_petugas_${id}`) || '';
    const unseenSet = new Set(unseenCodes);
    const revisionByMaterial = new Map();
    for (const v of voidedEntries || []) {
      if (v.voided_by === currentPetugas) continue;
  ...
  function jumpToRevision(v) {
    ...
    setOpenRak('(belum pernah dihitung)');
    setRakSearch(v.material_code);
  }
