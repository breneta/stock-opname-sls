...
export default function InputPage() {
  ...
  async function handleSaveRak(rak, materials) {
    ...
    for (const m of materials) {
      ...
      const batchOptionsForMaterial = recountBatchOptions[m.material] || [];
      if (batchOptionsForMaterial.length > 1 && !resolvedBatch) {
        setRakError(`${m.material}: Batch wajib dipilih (ada lebih dari satu batch di Master Data).`);
        return;
      }
      ...
      const base = {
        ...
        batch: resolvedBatch?.batch || null,
                        ...
                        <div className="mt-2">
                          ...
                          {batchOptions.length > 1 ? (
                            <select
                              className="input-field"
                              value={row.batch || ''}
                              onChange={(e) => updateRakRow(m.material, { batch: e.target.value || undefined })}
                            >
                              <option value="" disabled>Pilih batch...</option>
                              {batchOptions.map((o) => <option key={o.batch} value={o.batch}>{o.batch}</option>)}
                            </select>
                          ) : (
                            <div className="input-field flex items-center bg-paper text-ink/70">
                              {resolvedBatch?.batch || (batchOptions.length === 0 ? 'Tidak ada di SAP' : '-')}
                            </div>
                          )}
                          ...
                          {openHistoryFor === m.material && (
                            <div className="mt-2 space-y-2 rounded-lg border border-warn/30 bg-warn/5 p-2 text-xs">
                              <p className="text-ink/50">
                                Kalau entry lama rak/batch-nya salah, bisa dibatalkan langsung — material ini sudah di-approve untuk recount.
                              </p>
