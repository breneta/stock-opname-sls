            ...
            <>
              <div className="card p-5">
                <div className="mb-2 flex items-center gap-1.5 text-sm font-medium">
                  ...
                  <InfoTooltip text="Progress = Qty Input ÷ Qty SAP, dihitung terpisah per Unit of Measure supaya satuan BOX/PC/dll tidak tercampur. Jumlah Rak yang sudah disentuh dan jumlah Material ditampilkan di bawah sebagai detail tambahan." />
                </div>
                {rimpilanStats.rimpilanQtyByUom.length === 0 ? (
                  <div className="h-2.5 w-full overflow-hidden rounded-full bg-line">
                    <div className="h-full rounded-full bg-slate-850 transition-all" style={{ width: '0%' }} />
                  </div>
                ) : (
                    ...
                    {rimpilanStats.rimpilanQtyByUom.map((r) => (
                      <div key={r.uom}>
                        <div className="mb-1 flex items-center justify-between text-xs">
                          <span className="font-mono font-medium text-ink/70">{r.uom}</span>
                          <span className="font-mono text-ink/50">{fmt(r.qtyInput)} / {fmt(r.qtySap)} ({r.pct}%)</span>
                        </div>
                        <div className="h-2.5 w-full overflow-hidden rounded-full bg-line">
                          <div
                            className={`h-full rounded-full transition-all ${r.pct >= 100 ? 'bg-good' : 'bg-slate-850'}`}
