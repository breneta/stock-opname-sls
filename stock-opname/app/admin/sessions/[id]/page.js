...
export default function SessionHubPage() {
  ...
  async function load() {
    ...
    // "Belum Diinput" as a Qty — total Qty SAP milik material yang belum
    // discan sama sekali. Ini berkurang tiap ada input baru masuk.
        ...
        <>
          ...
          {stats.qtyByUom.length > 0 && (
            <div className="card p-5">
              ...
              <table className="w-full text-left text-sm">
                <thead className="text-xs text-ink/50">
                  <tr>
                    ...
                    <th className="pb-2 pr-3 text-right">Qty Belum Diinput</th>
                ...
                <tbody>
                  {stats.qtyByUom.map((r) => (
                    <tr key={r.uom} className="border-t border-line">
                      ...
                      <td className="py-2 pr-3 text-right font-mono text-warn">{r.qtyBelum || 0}</td>
              ...
              <p className="mt-2 text-xs text-ink/40">Qty Belum Diinput otomatis berkurang tiap ada material baru yang diinput petugas.</p>
            </div>
          )}
                ...
                <tbody>
                  {stats.selisihPreview.map((r, i) => (
                    <tr key={i} className="border-t border-line">
                      ...
                      <td className="py-2 pr-3 text-right font-mono">{r.qty_sap} {r.base_uom}</td>
                      <td className="py-2 pr-3 text-right font-mono">{r.total_qty_fisik} {r.base_uom}</td>
                      <td className={`py-2 pr-3 text-right font-mono ${r.selisih > 0 ? 'text-warn' : 'text-bad'}`}>{r.selisih}</td>
