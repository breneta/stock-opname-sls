...
export default function SessionHubPage() {
  ...
  const [cancellingRecount, setCancellingRecount] = useState(false);
  // How many entries (Normal + Rimpilan) already carry the CURRENT active
  // round number — if this is 0, "Mulai Recount" can be fully undone
  // (misclick, wrong material list, etc.) with no data left behind. Once a
  // petugas has actually saved something under this round, undoing it would
  // strand real input, so only "Selesaikan" is offered from then on.
  const [activeRoundEntryCount, setActiveRoundEntryCount] = useState(0);
  ...
  async function load() {
    ...
    const [{ data: sessionData }, sapData, entries, rimpilanSapData, rimpilanEntries] = await Promise.all([
      ...
      fetchAll(() => supabase.from('so_entries').select('*').eq('session_id', id).is('voided_at', null)),
      fetchAll(() => supabase.from('rimpilan_sap_data').select('*').eq('session_id', id)),
      fetchAll(() => supabase.from('rimpilan_entries').select('*').eq('session_id', id).is('voided_at', null)),
    ]);
    ...
    // Primary progress metric for Rimpilan is Rak-based, not material-count
    // based — petugas physically walk rak by rak here (accordion), and a
    // rak with 20 materials counted as "1 material done" out of many would
    // undersell real progress. Material count + Qty breakdown are still
    // shown as secondary detail below.
    const rakProgress = totalRaks > 0 ? Math.round((raksTouched / totalRaks) * 100) : 0;

    const rimpilanByUom = new Map();
    const getRUom = (uom) => {
      const key = uom || '(tanpa satuan)';
      if (!rimpilanByUom.has(key)) rimpilanByUom.set(key, { uom: key, qtySap: 0, qtyInput: 0 });
      return rimpilanByUom.get(key);
    };
    for (const r of rimpilanSapData || []) getRUom(r.base_uom).qtySap += Number(r.qty) || 0;
    for (const r of rimpilanResult.rows) getRUom(r.base_uom).qtyInput += r.total_qty_fisik;
    const rimpilanQtyByUom = [...rimpilanByUom.values()]
      .map((u) => ({ ...u, pct: u.qtySap > 0 ? Math.round((u.qtyInput / u.qtySap) * 100) : 0 }))
      .sort((a, b) => a.uom.localeCompare(b.uom));
    ...
    // Snapshot the actual numbers at the moment recount starts — this rides
    // along in recount_material_codes so the Input pages can show petugas
    // exactly what's off (Qty SAP / Qty Tercatat / Selisih) with zero extra
    // queries. Deliberately NOT shown during the FIRST count (that stays a
    // blind count so petugas don't anchor to the SAP number) — only once
    // Accounting has actually decided this material needs a recount.
    ...
    const activeRound = sessionData?.active_recount_round || 0;
    const roundEntryCount = activeRound > 0
      ? (entries || []).filter((e) => (e.recount_round || 0) === activeRound).length
        + (rimpilanEntries || []).filter((e) => (e.recount_round || 0) === activeRound).length
      : 0;
    setActiveRoundEntryCount(roundEntryCount);
  ...
  // Pure record-keeping, no validation/workflow — just a place to note down
  // the two reference numbers SAP generates once the physical inventory
  // posting is done there, so this session points straight to the SAP side
  // instead of relying on someone's memory or a side spreadsheet.
  async function handleSaveSapRefs(pidNumber, materialDocumentNumber) {
    const { error: saveError } = await supabase
      .from('so_sessions')
      .update({ pid_number: pidNumber.trim() || null, material_document_number: materialDocumentNumber.trim() || null })
      .eq('id', id);
    if (saveError) {
  ...
  // Full undo — only offered while activeRoundEntryCount is 0 (no petugas
  // has saved anything under this round yet), so there's nothing to strand.
  // Unlike "Selesaikan", this also rolls active_recount_round back down and
  // removes the so_recount_rounds row, as if "Mulai Recount" was never
  // clicked — for the misclick / "salah pilih material" case.
  async function handleCancelRecount() {
    if (activeRoundEntryCount > 0) return;
    const round = session.active_recount_round || 0;
    if (!confirm(`Batalkan Recount Round ${round}? Daftar material yang dikunci akan dihapus, seperti belum pernah menekan "Mulai Recount".`)) return;

    setCancellingRecount(true);
    const { error: deleteRoundError } = await supabase
      .from('so_recount_rounds')
      .delete()
      .eq('session_id', id)
    ...
    const { error: updateError } = await supabase
      .from('so_sessions')
      .update({ active_recount_round: Math.max(round - 1, 0), recount_material_codes: [] })
      .eq('id', id);

    setCancellingRecount(false);
    if (deleteRoundError || updateError) {
      alert('Gagal membatalkan recount: ' + (deleteRoundError?.message || updateError?.message));
      return;
    }
    load();
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="-mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs">
        <Link href={`/admin/sessions/${id}/reconciliation`} className="font-medium text-slate-850 hover:underline">
          Rekonsiliasi →
        </Link>
        <Link href={`/admin/sessions/${id}/entries`} className="font-medium text-slate-850 hover:underline">
          Kelola Entry (koreksi input salah) →
        </Link>
      </div>

      {session.status === 'closed' && (
        <SapDocumentRefsCard session={session} onSave={handleSaveSapRefs} />
      )}
        ...
        recountActive ? (
          <div className="card flex items-center justify-between border-warn/40 bg-warn/10 p-4">
            <div className="text-sm">
              ...
              {activeRoundEntryCount === 0 ? (
                <div className="mt-0.5 text-xs text-ink/40">Belum ada input masuk di round ini — masih bisa dibatalkan sepenuhnya.</div>
              ) : (
                <div className="mt-0.5 text-xs text-ink/40">{activeRoundEntryCount} input sudah masuk di round ini — tidak bisa dibatalkan, hanya bisa diselesaikan.</div>
              )}
            </div>
            <div className="flex shrink-0 gap-2">
              {activeRoundEntryCount === 0 && (
                <button onClick={handleCancelRecount} disabled={cancellingRecount} className="btn-ghost text-xs text-bad">
                  {cancellingRecount ? 'Membatalkan...' : 'Batalkan'}
                </button>
              )}
              <button onClick={handleStopRecount} className="btn-ghost text-xs">Selesaikan Recount</button>
            </div>
          </div>
        ...
        <>
          {!rimpilanStats.hasRimpilanData ? (
            <div className="card border-amber/40 bg-amber/10 p-5">
              ...
              <p className="mt-1 text-sm text-ink/60">
                Upload Data Master Rimpilan (material + rak) terlebih dahulu. Level dipilih petugas
                sendiri saat input, bukan bagian dari file ini.
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                <Link href={`/admin/sessions/${id}/upload/rimpilan`} className="btn-primary inline-flex">
                  Upload Data Rimpilan
                </Link>
                <DownloadRimpilanTemplateButton />
              </div>
            </div>
          ) : (
            <>
              <div className="card p-5">
                <div className="mb-2 flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-sm font-medium">
                    ...
                    <InfoTooltip text="Progress = jumlah Rak yang sudah disentuh (minimal 1 input) ÷ Total Rak di Data Master Rimpilan. Berbasis Rak karena petugas menghitung per-rak, bukan satu-satu — jumlah Material dan Qty tetap ditampilkan di bawah sebagai detail." />
                  </span>
                  <span className="font-mono text-lg font-semibold text-ink">{rimpilanStats.rakProgress}%</span>
                </div>
                <div className="h-2.5 w-full overflow-hidden rounded-full bg-line">
                  <div
                    className={`h-full rounded-full transition-all ${rimpilanStats.rakProgress >= 100 ? 'bg-good' : 'bg-slate-850'}`}
                    style={{ width: `${rimpilanStats.rakProgress}%` }}
                  />
                </div>
                <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs text-ink/50">
              ...
              {rimpilanStats.rimpilanQtyByUom.length > 0 && (
                <div className="card p-5">
                  <div className="mb-3 text-sm font-medium">Ringkasan Qty Rimpilan</div>
                  <table className="w-full text-left text-sm">
                    <thead className="text-xs text-ink/50">
                      <tr>
                        <th className="pb-2 pr-3">UoM</th>
                    ...
                    <tbody>
                      {rimpilanStats.rimpilanQtyByUom.map((r) => (
                        <tr key={r.uom} className="border-t border-line">
                          <td className="py-2 pr-3 font-mono">{r.uom}</td>
                          <td className="py-2 pr-3 text-right font-mono">{fmt(r.qtySap)}</td>
                          <td className="py-2 pr-3 text-right font-mono">{fmt(r.qtyInput)}</td>
                          <td className="py-2 pr-3 text-right font-mono">{r.pct}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
...
// Record-only — no validation, no workflow. Shown after a session is
// closed so Accounting can note down the SAP-side reference numbers once
// the physical inventory posting is done there (PID from the count, the
// Material Document from the posted adjustment).
function SapDocumentRefsCard({ session, onSave }) {
  const [pid, setPid] = useState(session.pid_number || '');
  const [matDoc, setMatDoc] = useState(session.material_document_number || '');
  const [saving, setSaving] = useState(false);

  const dirty = pid !== (session.pid_number || '') || matDoc !== (session.material_document_number || '');

  async function handleSave() {
    setSaving(true);
    await onSave(pid, matDoc);
    setSaving(false);
  }
  ...
  return (
    <div className="card p-4">
      <div className="flex items-center gap-1.5 text-sm font-medium">
        Dokumen SAP
        <InfoTooltip text="Pencatatan saja, tidak ada validasi format — isi setelah posting fisik inventory selesai di SAP." />
      </div>
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        <div>
          <label className="label-field">Physical Inventory Document (PID)</label>
          <input className="input-field font-mono" value={pid} onChange={(e) => setPid(e.target.value)} placeholder="Contoh: 100001234" />
        </div>
        <div>
          <label className="label-field">Material Document</label>
          <input className="input-field font-mono" value={matDoc} onChange={(e) => setMatDoc(e.target.value)} placeholder="Contoh: 4900012345" />
        </div>
      </div>
      {dirty && (
        <button onClick={handleSave} disabled={saving} className="btn-primary mt-3 text-xs">
...
// Small secondary button (not the primary "Upload" CTA) that generates an
// .xlsx template on the fly — no Level column, matches upload/rimpilan's
// COLUMN_MAP exactly, with 2 example rows so it's obvious what goes where.
function DownloadRimpilanTemplateButton() {
  const [downloading, setDownloading] = useState(false);

  async function handleDownload() {
    setDownloading(true);
    const mod = await import('../../../../lib/rimpilanTemplate');
    mod.downloadRimpilanTemplate();
    setDownloading(false);
  }

  return (
    <button onClick={handleDownload} disabled={downloading} className="btn-ghost inline-flex">
      {downloading ? 'Menyiapkan...' : 'Download Template'}
    </button>
  );
}
