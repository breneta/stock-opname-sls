...
export default function ReconciliationPage() {
  ...
  async function load() {
    ...
    const { rows } = buildReconciliation(sapData || [], sapEntries);
    setRows(rows);
    setLoading(false);
  }
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <h1 className="mt-1 text-xl font-semibold tracking-tight">Rekonsiliasi SAP</h1>
        </div>
        <Link href={`/admin/sessions/${id}/reconciliation-rimpilan`} className="text-xs font-medium text-slate-850 hover:underline">
          Lihat Rekonsiliasi Rimpilan →
        </Link>
      </div>
        ...
        <div className="card overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={9} className="px-4 py-8 text-center text-ink/40">Tidak ada data.</td></tr>
              )}
            </tbody>
          </table>
        </div>
