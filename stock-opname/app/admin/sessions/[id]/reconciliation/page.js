...
export default function ReconciliationPage() {
  ...
  async function load() {
    setLoading(true);
    const [sapData, entries, rimpilanEntries] = await Promise.all([
    ...
    // Selisih di sini = gabungan Normal SO + Rimpilan (lihat lib/reconciliation.js)
    // — angka yang sama dengan yang dipakai tombol "Mulai Recount" dan Export Excel.
    const { rows } = buildCombinedReconciliation(sap, entries || [], rimpilanEntries || []);
    setRows(rows);
    setLoading(false);
  }
  ...
  const kondisiSet = useMemo(() => {
    ...
    rows.forEach((r) => r.entries.forEach((e) => {
      if (e.kondisi_barang) s.add(e.kondisi_barang);
      if (e.keterangan_khusus) s.add(e.keterangan_khusus);
    }));
    return [...s];
  }, [rows]);
  ...
  const filtered = rows.filter((r) => {
    ...
    if (filterKondisi && !r.entries.some((e) => e.kondisi_barang === filterKondisi || e.keterangan_khusus === filterKondisi)) return false;
            ...
            <table className="w-full text-left text-xs">
              <thead className="text-ink/50">
                <tr>
                  ...
                  <th className="pb-2 pr-3">Kondisi / Keterangan</th>
                  <th className="pb-2 pr-3">Catatan</th>
                </tr>
              </thead>
              <tbody>
                {detailRow.entries.map((e) => {
                  // __source is stamped by buildCombinedReconciliation and is
                  // the only reliable signal — both Normal and Rimpilan now
                  // carry a `level` field, so that alone can't tell them apart.
                  const isRimpilan = e.__source === 'rimpilan';
                  return (
                    <tr key={e.id} className="border-t border-line">
                      <td className="py-2 pr-3 whitespace-nowrap">{new Date(e.created_at).toLocaleString('id-ID')}</td>
                      <td className="py-2 pr-3">
                        <span className={`badge ${isRimpilan ? 'bg-amber/20 text-warn' : 'bg-slate-850/10 text-ink'}`}>
                          {isRimpilan ? 'Rimpilan' : 'Normal'}
                        </span>
                      </td>
                      <td className="py-2 pr-3">{e.petugas_nama}</td>
                      <td className="py-2 pr-3 font-mono">{e.nomor_rak}{e.level ? ` (Lv ${e.level})` : ''}</td>
                      <td className="py-2 pr-3 text-right font-mono">{e.qty_fisik}</td>
                      <td className="py-2 pr-3 font-mono">{e.base_uom}</td>
                      <td className="py-2 pr-3">{e.kondisi_barang || e.keterangan_khusus || '—'}</td>
                      <td className="py-2 pr-3">{e.catatan || e.keterangan_catatan}</td>
                    </tr>
                  );
                })}
              </tbody>
