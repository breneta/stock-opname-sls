...
// Same template shape as the Normal SO SAP upload, plus Nomor Rak + Level
// — those two are what let the Rimpilan Input page group materials into
// the per-rak accordion without petugas having to type anything.
const COLUMN_MAP = {
  material: ['material'],
  batch: ['batch'],
...
export default function UploadRimpilanMasterPage() {
  ...
  async function handleFile(e) {
    ...
    try {
      ...
      // Level opsional (default 1 kalau kosong) — Material Description
      // juga opsional. Sisanya wajib, termasuk Nomor Rak.
      ...
      for (const r of rawRows) {
        ...
        const levelRaw = r.level === '' || r.level === undefined ? 1 : Number(r.level);
        const level = Number.isFinite(levelRaw) ? levelRaw : NaN;
        ...
        if (!Number.isFinite(level) || level < 1 || level > 7) {
          invalid.push({ ...r, reason: `Level "${r.level}" harus angka 1-7` });
          continue;
        }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      <div>
        ...
        <h1 className="mt-1 text-xl font-semibold tracking-tight">Upload Data Master Rimpilan</h1>
        <p className="mt-1 text-sm text-ink/60">
          Template sama seperti Data SAP Normal, ditambah kolom Nomor Rak dan Level (1-7). Kolom wajib:
          Material, Batch, Base Unit of Measure, Plant, Storage Location, Qty, Nomor Rak. Level opsional
          (default 1). Material Description opsional.
        </p>
      </div>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-line">
                  ...
                  <td className="px-3 py-2 font-mono">{r.level}</td>
              ...
              {!rowsLoading && rows.length === 0 && (
                <tr><td colSpan={7} className="px-3 py-6 text-center text-ink/40">
