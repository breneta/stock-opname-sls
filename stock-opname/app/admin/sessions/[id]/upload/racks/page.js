...
import { downloadRacksTemplate } from '../../../../../../lib/racksTemplate';

// No warehouse_code — 1 session = 1 warehouse already (the session itself
// is the scope), so a separate warehouse layer was redundant. Just a flat
// list of Rack codes that powers the Rak accordion on Input Rimpilan.
const COLUMN_MAP = {
...
export default function UploadWarehouseRacksPage() {
  ...
  async function handleDeleteRow(row) {
    if (!confirm(`Hapus rak "${row.rack_code}"?`)) return;
  ...
  async function handleDeleteAll() {
    if (!confirm(`Hapus SEMUA ${totalCount} rak di session ini?`)) return;
    setDeletingAll(true);
    ...
    setManageMessage({ type: 'success', text: 'Semua rak sudah dihapus.' });
    loadRows();
  }
  ...
  async function handleFile(e) {
    ...
    try {
      ...
      const missing = Object.entries(COLUMN_MAP)
        ...
        .map(([field]) => field);
      if (missing.length) {
        throw new Error(`Kolom wajib tidak ditemukan: ${missing.join(', ')} (rack_code)`);
      }
      ...
      for (const r of rawRows) {
        ...
        if (!rackCode) {
          invalid.push({ ...r, reason: 'rack_code kosong' });
          continue;
        }
        if (seen.has(rackCode)) continue; // silently dedupe within the file
        seen.add(rackCode);
        valid.push({ rack_code: rackCode });
      }
  ...
  async function handleUpload() {
    ...
    try {
      // upsert on (session_id, rack_code) so re-uploading the same/updated
      // file is idempotent instead of erroring on the unique constraint or
      // creating duplicates.
      const rowsToInsert = parsedRows.map((r) => ({ session_id: id, ...r }));
      const { error } = await supabase
        .from('warehouse_racks')
        .upsert(rowsToInsert, { onConflict: 'session_id,rack_code' });
    ...
    <div className="mx-auto max-w-2xl space-y-5">
      <div>
        ...
        <div className="mt-1 flex flex-wrap items-start justify-between gap-3">
          <h1 className="text-xl font-semibold tracking-tight">Upload Warehouse Racks</h1>
          <button onClick={downloadRacksTemplate} className="btn-ghost shrink-0">Download Template</button>
        </div>
        <p className="mt-1 text-sm text-ink/60">
          CSV/Excel dengan 1 kolom: <span className="font-mono">rack_code</span>. Daftar rak ini dipakai
          di halaman Input Rimpilan untuk mengelompokkan material per rak — otomatis terhubung ke session
          ini (1 session = 1 warehouse, jadi warehouse_code tidak perlu lagi).
        </p>
      </div>
        ...
        <div className="card border-warn/30 bg-warn/5 p-4 text-sm">
          ...
          <ul className="mt-2 max-h-32 space-y-0.5 overflow-y-auto text-xs text-ink/60">
            {invalidRows.slice(0, 20).map((r, i) => (
              <li key={i}>{r.rack_code || '(kosong)'} — {r.reason}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      {parsedRows.length > 0 && (
        <div className="card p-5">
          <div className="mb-3 text-sm font-medium">Preview ({parsedRows.length} rak unik)</div>
          ...
          <button onClick={handleUpload} disabled={uploading} className="btn-primary mt-4">
            {uploading ? 'Menyimpan...' : `Simpan ${parsedRows.length} Rak`}
          </button>
          {uploadResult?.ok && (
            <div className="mt-3 rounded-lg border border-good/30 bg-good/5 p-3 text-sm text-good">
              ✓ {uploadResult.count} rak tersimpan (baris yang sudah ada otomatis diperbarui).
            </div>
          )}
        </div>
      )}
      ...
      <div className="border-t border-line pt-6">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-lg font-semibold tracking-tight">Rak Saat Ini ({totalCount})</h2>
          {totalCount > 0 && (
        ...
        {rows.length === 0 ? (
          <div className="card p-6 text-center text-sm text-ink/40">Belum ada rak di session ini.</div>
        ) : (
