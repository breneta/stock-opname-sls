    ...
    const selisihResults = await Promise.all(
      sessions.map(async (s) => {
        const [sapData, normalEntries, rimpilanEntries] = await Promise.all([
          ...
          fetchAll(() => supabase.from('so_entries').select('material_code,batch,plant,storage_location,base_uom,qty_fisik,keterangan_khusus,recount_round,status_sap').eq('session_id', s.id).is('voided_at', null)),
          fetchAll(() => supabase.from('rimpilan_entries').select('material_code,batch,plant,storage_location,base_uom,qty_fisik,keterangan_khusus,recount_round').eq('session_id', s.id).is('voided_at', null)),
        ]);
