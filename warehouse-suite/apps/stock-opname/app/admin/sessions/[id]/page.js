...
export default function SessionHubPage() {
  ...
  async function load() {
    ...
    const sap = sapData || [];
    const sapEntries = (entries || []).filter((e) => e.source !== 'rimpilan');
    ...
    <div className="space-y-6">
      ...
      {!stats.hasSapData && (
        <div className="card border-amber/40 bg-amber/10 p-5">
          ...
          <p className="mt-1 text-sm text-ink/60">
            Upload Data SAP untuk mengaktifkan progress & rekonsiliasi jalur SAP. Jalur Rimpilan
            tetap bisa dipakai karena datanya berasal dari Master Material Rimpilan, bukan dari sini.
          </p>
        ...
        <>
          <div className="card p-5">
            <div className="mb-2 flex items-center justify-between text-sm">
              <span className="font-medium">Progress Stock Opname (SAP)</span>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <ActionLink
          ...
          title="Rekonsiliasi SAP"
          desc="Bandingkan hasil hitung dengan Data SAP"
        />
        <ActionLink
          href={`/admin/sessions/${id}/reconciliation-rimpilan`}
          title="Rekonsiliasi Rimpilan"
          desc="Bandingkan hasil hitung dengan stok Master Rimpilan"
        />
        <ActionLink
        ...
        />
        <ExportAction sessionId={id} sessionName={session.name} plant={session.plant} />
      </div>
    </div>
  );
}
...
function ExportAction({ sessionId, sessionName, plant }) {
  const [exporting, setExporting] = useState(false);

  async function handleExport() {
    setExporting(true);
    const mod = await import('../../../../lib/exportExcel');
    await mod.exportSessionToExcel(sessionId, sessionName, plant);
    setExporting(false);
  }
  ...
  return (
    <button onClick={handleExport} disabled={exporting} className="card block p-4 text-left transition hover:border-slate-850/30 hover:shadow-md disabled:opacity-60">
      ...
      <div className="mt-0.5 text-sm text-ink/50">SAP & Rimpilan — Summary, Selisih, Detail Scan</div>
    </button>
  );
}
