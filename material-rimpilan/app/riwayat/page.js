...
export default function RiwayatPage() {
  ...
  const filtered = tx.filter((t) => {
    ...
    if (filterPlant && t.mr_materials?.plant !== filterPlant) return false;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        />
        <select className="input-field max-w-[180px]" value={filterPlant} onChange={(e) => setFilterPlant(e.target.value)}>
            ...
            <tbody>
              {filtered.map((t) => (
                <tr key={t.id} className="border-t border-line">
                  ...
                  <td className="px-4 py-2.5">{t.mr_materials?.plant}</td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-ink/40">Tidak ada data.</td></tr>
              )}
            </tbody>
          </table>
        </div>
